...
Add issues
	...
	Title Should Be    Scrum Board
	Click Link    addIssues
	
Input level issues
	Location Should Be    ${ADD_ISSUES}
	Title Should Be    Scrum Board
	Click Element     xpath: //*[contains(text(), "ด่วน")]     
	Click Button    add_Issues
	
Input input name
	Location Should Be    ${ADD_ISSUES}
	Title Should Be    Scrum Board
	Input Text    issues_topic    code error
	Click Button    add_Issues

Input input Description
    Location Should Be    ${ADD_ISSUES}
	Title Should Be    Scrum Board
	Input Text    issues_content    code error
	Click Button    add_Issues

Input name description  issues
	Location Should Be    ${ADD_ISSUES}
	Title Should Be    Scrum Board
	Input Text    issues_topic    code error
	Input Text    issues_content    code error
